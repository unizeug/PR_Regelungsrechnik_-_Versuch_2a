...
// Aufruf: [ w, [db], [phi] ] = bode_w(sl, [w_min, w_max], [title], [show_margins], [n]);
//      sl =           Lineares System
...
function varargout=bode_w(sl, varargin)
  [lhs, rhs] = argn();
  // Defaults
  exponent = [-2 ; 4 ];
  title_ = '';
  ...
  n = 1000;
  if (rhs > 1) then
    if (rhs == 3 | rhs == 4 | rhs == 5 | rhs == 6) then 	// w_min und w_max
    ...
    if (rhs == 4 | rhs == 5 | rhs == 6) then 	// w_min und w_max
        title_ = varargin(3);
    end
    if (rhs == 5) then
      s_m = varargin(4);
  ...
  //scf();
  subplot(211);
    plot2d(f*2*%pi,db, style=[1],logflag='ln' );
    e=gce();
    ...
    if (~(isempty(f_phase) | ~(f_phase > f(1) & f_phase < f(length(f))) )) then
      ...
      plot2d([5 5], [figure_size(2,1) figure_size(2,2)], style=3);
      plot2d([220 220], [figure_size(2,1) figure_size(2,2)], style=3);
      e=gce(); e.children(1).line_style=4;      
    end
  subplot(212);
    plot2d(f*2*%pi,phi, style=[1],logflag='ln');
    e=gce();
    e.children(1).line_style=1;
    xgrid
